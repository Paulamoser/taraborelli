        ...
        for rec in self:
            if rec.move_type == 'out_invoice' or rec.move_type == 'out_refund':
                if rec.invoice_line_ids:
                    if rec.partner_id:
                        if len(rec.partner_id.alicuot_per_arba_ids) > 0:
                            return True
    ...
    def calculate_perceptions(self):
        if self.move_type == 'out_invoice' or self.move_type == 'out_refund':
            if self.invoice_line_ids:
                if self.partner_id:
                    if len(self.partner_id.alicuot_per_arba_ids) > 0:
                        #Busco el impuesto a utilizar para la percepcion
                        imp_per_arba = self.env['account.tax'].browse(int(self.env['ir.config_parameter'].sudo().get_param('l10n_ar_tax.arba')))

                        # Cambio el amount del impuesto por el valor que tenga el partner en el padron
                        imp_per_arba.amount = self.partner_id.alicuot_per_arba_ids.filtered(lambda l: l.padron_activo == True)[0].a_per
                        ...
                        for iline in self.invoice_line_ids:
                            ...
                            for tax in iline.tax_ids:
                                if str(imp_per_arba.id) == str(tax.id)[-2:]:
                                    _tiene_precepcion = 1
                            if not _tiene_precepcion:
                                iline.write({'tax_ids': [(4, imp_per_arba.id)]})
